...
The fair objection to everything above is that none of it obviously needs RxJS. Four of the five principles would survive on a plain normalized store and a subscribe callback. The difference is in what you have to build on top of it.
...
**Timing stops being React's business.** With hooks the render loop is your scheduler: async coordination has to be written as effects that run after a render, and every intermediate value you did not want still costs one. Streams sit outside that loop, and each one can be tuned on its own. Filter a list quickly and the requests for `a`, `ab`, `abc` overlap — `abc` returns first, `ab` lands after it, and the view ends up showing a query the user has already moved past. `switchMap` drops the stale response before React hears about it at all. Debounce one param, retry one load, coalesce a firehose of live patches into a single render per frame — each of those is local to its own stream and changes nothing about how the rest of the app renders.
