...
function sameLineGroups(a: number[][], b: number[][]): boolean {
...
type HoverMode = "none" | "diffusionOneLine" | "diffusionMultiLine" | "waveOneLine" | "waveMultiLine";
...
export const GlitchCharReveal = memo(function GlitchCharReveal(props: Partial<typeof COMPONENT_DEFAULTS>) {
  props = { ...COMPONENT_DEFAULTS, ...props };
  const { words, enterAnimation, hoverAnimation, color, font } = props;

  const glitchColor = color;

  // ── Enter props ───────────────────────────────────────────────────────────
  const enterMode: EnterMode = (enterAnimation?.mode ?? "oneLine") as EnterMode;
  const enterEase = enterAnimation?.ease ?? {
    type: "tween",
    duration: 2,
    ease: "easeOut",
  };
  ...
  const spanStyle = typeface ? Object.fromEntries(Object.entries(typeface).filter(([k]) => k !== "textAlign")) : {};
  ...
  const detectLines = () => {
    ...
    // Only update when the grouping actually changed. detectLines is called by
    // a ResizeObserver, and the enter animation itself mutates layout (chars are
    // toggled with display:none as the line builds). Setting a fresh array on
    // every measurement would hand the enter-animation effect a new `lineGroups`
    // reference, cancel it, and restart it from the first character — an infinite
    // loop that leaves the first glyph perpetually scrambling.
    setLineGroups((prev) => (sameLineGroups(prev, allLines) ? prev : allLines));
  };
  ...
  useEffect(() => {
    ...
    const animateChar = async (globalWi: number, ci: number, char: string, targetEnd: number) => {
      ...
      const desiredFrames =
        scrambleIntensity === 0 ? 0 : 1 + Math.floor(Math.random() * Math.round(scrambleIntensity / 7));
